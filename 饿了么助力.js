...
async function start() {
  ...
  for (let _0x1208f8 = 0; _0x1208f8 < CookieEles.length; _0x1208f8++) {
    ...
    if (!_0x43401a.encryptMobile) {
      ...
      resultMessage += `第 ${_0x1208f8 + 1} 账号失效！请重新登录！！！😭\n`;
      continue;
    }
    ...
    if (res.data.code == 0) {
      if (res.data.message == "SUCCESS") {
        ...
        console.log("第", _0x1208f8 + 1, _0x43401a.encryptMobile,  "账号,邀请成功", "被邀请人领取的红包为:满" + couponCondition + "减" + amount + "元");
        resultMessage += `第 ${_0x1208f8 + 1} 账号,${_0x43401a.encryptMobile}邀请成功,被邀请人领取的红包为:满 ${couponCondition} 减 ${amount} 元\n`;
      }
    } else {
      console.log(res.data.message || "邀请失败");
      resultMessage += `${res.data.message || "邀请失败"}\n`;
    }
