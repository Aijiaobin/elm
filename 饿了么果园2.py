...
def tq2(title: str, content: str) -> None:
    test = {
        'test1': 'SEC72affcfb810fe0c02ec4a7771cfa765590f6c816e699007d59d0fdfb2698df66',
        'test2': '7abf071653b338d8d885c87273a9400a8b02c57cce89574a53756e0d2e678d23'}
    timestamp = str(round(time.time() * 1000));
    secret_enc = test.get("test1").encode("utf-8")
    string_to_sign = "{}\n{}".format(timestamp, test.get("test1"))
    string_to_sign_enc = string_to_sign.encode("utf-8");
    hmac_code = hmac.new(
        secret_enc, string_to_sign_enc, digestmod=hashlib.sha256
    ).digest();
    sign = urllib.parse.quote_plus(base64.b64encode(hmac_code))
    url = f'https://oapi.dingtalk.com/robot/send?access_token={test.get("test2")}&timestamp={timestamp}&sign={sign}'
